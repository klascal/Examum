...
router.get('/docent', requireAuth('docent'), (req, res) => {
  ...
  const examens = req.session.user.role === 'admin'
    ? db.prepare(`
        SELECT e.*,
...
router.get('/docent/resultaten/:examen_id', requireAuth('docent'), (req, res) => {
  const examen = req.session.user.role === 'admin'
    ? db.prepare('SELECT * FROM examens WHERE id = ?').get(req.params.examen_id)
    : db.prepare('SELECT * FROM examens WHERE id = ? AND created_by = ?').get(req.params.examen_id, req.session.user.id);

  if (!examen) {
    return res.status(403).render('error', { message: 'Geen toegang tot dit examen', title: 'Verboden' });
  }

  const resultaten = db.prepare(`
...
router.get('/docent/open-vragen/:examen_id', requireAuth('docent'), (req, res) => {
  const examen = req.session.user.role === 'admin'
    ? db.prepare('SELECT * FROM examens WHERE id = ?').get(req.params.examen_id)
    : db.prepare('SELECT * FROM examens WHERE id = ? AND created_by = ?').get(req.params.examen_id, req.session.user.id);
  ...
  const openVragenMapped = openVragen.map(v => ({
    ...
    beoordeling: v.beoordeling_id
      ? { id: v.beoordeling_id, score: v.beoordeling_score, feedback: v.beoordeling_feedback }
      : null
  }));
...
router.post('/docent/beoordeel', requireAuth('docent'), (req, res) => {
  ...
  if (!antwoord_id || score === undefined || score === '') {
    return res.redirect('/docent/open-vragen/' + examen_id + '?error=Ongeldige+invoer');
  }

  // Verifieer dat het antwoord bij een examen van deze docent hoort
  const check = req.session.user.role === 'admin'
    ? db.prepare(`
  ...
  db.prepare('DELETE FROM docent_beoordelingen WHERE antwoord_id = ?').run(antwoord_id);

  db.prepare(`
    INSERT INTO docent_beoordelingen (id, antwoord_id, docent_id, score, feedback)
    VALUES (?, ?, ?, ?, ?)
  `).run(generateId(), antwoord_id, req.session.user.id, scoreVal, feedback ? feedback.trim() : null);

  const isCorrect = scoreVal > 0 ? 1 : 0;
  db.prepare('UPDATE antwoorden SET is_correct = ? WHERE id = ?').run(isCorrect, antwoord_id);

  const antwoord = db.prepare('SELECT kandidaat_examen_id FROM antwoorden WHERE id = ?').get(antwoord_id);
  if (antwoord) {
...
function recalculateResult(keId) {
  const ke = db.prepare('SELECT examen_id FROM kandidaat_examen WHERE id = ?').get(keId);
  if (!ke) return;

  const totaalRow = db.prepare(
    "SELECT COUNT(*) as count FROM vragen WHERE examen_id = ? AND is_info = 0"
  ).get(ke.examen_id);

  const antwoorden = db.prepare(
    "SELECT is_correct FROM antwoorden WHERE kandidaat_examen_id = ?"
  ).all(keId);
  ...
  const examen = db.prepare('SELECT min_score FROM examens WHERE id = ?').get(ke.examen_id);
