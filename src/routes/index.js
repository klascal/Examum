...
router.get('/login', (req, res) => {
  if (req.session.user) {
    return res.redirect(req.session.user.role === 'admin' ? '/admin' : '/docent');
  }
...
router.post('/login', loginLimiter, (req, res) => {
  ...
  if (!username || !password) {
    return res.render('login', { title: 'Inloggen', layout: false, error: 'Vul alle velden in' });
  }

  // Zoek op username OF email (voor docenten die geen username hebben)
  const user = db.prepare('SELECT * FROM users WHERE (username = ? OR email = ?) AND actief = 1').get(username, username);
  ...
  const userData = { id: user.id, naam: user.naam, username: user.username, role: user.role };

  // Regenereer sessie om session fixation te voorkomen
  req.session.regenerate((err) => {
    if (err) return res.status(500).render('error', { message: 'Sessie fout', title: 'Fout' });
...
router.get('/kandidaat-login', (req, res) => {
  ...
  res.render('student/login', { title: 'Kandidaat Login', layout: false });
});
...
router.post('/kandidaat-login', loginLimiter, (req, res) => {
  ...
  if (!inlogcode || !wachtwoord) {
    return res.render('student/login', { title: 'Kandidaat Login', layout: false, error: 'Vul alle velden in' });
  }
