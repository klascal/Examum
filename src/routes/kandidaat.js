...
router.get('/examen', requireKandidaat, (req, res) => {
  ...
  const examen = db.prepare('SELECT * FROM examens WHERE id = ?').get(ke.examen_id);
  const aantalVragen = db.prepare(
    "SELECT COUNT(*) as count FROM vragen WHERE examen_id = ? AND is_info = 0"
  ).get(ke.examen_id).count;
  const aantalInfo = db.prepare(
    "SELECT COUNT(*) as count FROM vragen WHERE examen_id = ? AND is_info = 1"
  ).get(ke.examen_id).count;
  const aantalOpen = db.prepare(`
...
router.post('/examen/start', requireKandidaat, (req, res) => {
  ...
  const current = db.prepare('SELECT status FROM kandidaat_examen WHERE id = ?').get(assignment.id);
  if (!current) {
  ...
  if (current.status === 'niet_gestart') {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?)
    `).run(generateId(), assignment.id, 'exam_start', JSON.stringify({ ip: clientIP }));
  }
...
router.get('/examen/vraag/:nr', requireKandidaat, (req, res) => {
  ...
  if (ke.gestart) {
    ...
    const verstrekenSec = Math.floor((Date.now() - gestartDate.getTime()) / 1000);
...
router.post('/examen/antwoord', requireKandidaat, (req, res) => {
  ...
  const current = db.prepare('SELECT status, examen_id FROM kandidaat_examen WHERE id = ?').get(assignment.id);
  ...
  const vraag = db.prepare(
    "SELECT id FROM vragen WHERE id = ? AND examen_id = ?"
  ...
  const opties = db.prepare(
    "SELECT label, is_correct FROM antwoord_opties WHERE vraag_id = ?"
  ).all(vraag_id);
...
router.get('/examen/resultaat', requireKandidaat, (req, res) => {
  ...
  const resultaat = db.prepare(
    "SELECT * FROM resultaten WHERE kandidaat_examen_id = ?"
  ).get(ke.id);
